...
fn inventory_grid_cursor_sprites_follow_grid_coordinates() {
    ...
    engine.set_mem(0xF7, 0x03);

    game::update_inventory_grid_cursor_sprites(&mut engine, &mut r);

    assert_eq!(engine.mem(0x0297), 0x46);
    assert_eq!(engine.mem(0x0293), 0x3E);
    assert_eq!(engine.mem(0x0290), 0x99);
    assert_eq!(engine.mem(0x0294), 0x99);
...
fn inventory_list_cursor_uses_low_five_bits() {
    ...
    assert_eq!(engine.mem(0x0280), 0x69);
    assert_eq!(engine.mem(0x0284), 0x69);
    assert_eq!(engine.mem(0x0283), 0xBE);
    assert_eq!(engine.mem(0x0287), 0xC6);
}
...
fn selecting_inventory_grid_entry_writes_to_scrolling_list() {
    ...
    assert_eq!(engine.mem(0x0325), 0x07);
