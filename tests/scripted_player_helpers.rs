...
fn scripted_player_projection_applies_x_and_y_deltas() {
    ...
    engine.state.set_player_x_fine(0x20);
...
fn scripted_player_bounds_reject_bottom_and_right_edges() {
    ...
    engine.state.set_scratch2(0xA0);
    engine.state.set_indirect_ptr_lo(0xF0);

    game::check_scripted_player_bounds(&mut engine, &mut r);
    assert_eq!(r.carry, 0);

    engine.state.set_scratch2(0xA1);
    game::check_scripted_player_bounds(&mut engine, &mut r);
    assert_eq!(r.carry, 1);

    engine.state.set_scratch2(0x80);
    engine.state.set_indirect_ptr_lo(0xF1);
    game::check_scripted_player_bounds(&mut engine, &mut r);
    assert_eq!(r.carry, 1);
}
...
fn scripted_player_move_retries_vertical_delta_toward_bounds() {
    ...
    engine.state.set_player_x_fine(0x20);
    engine.state.set_player_y(0x9F);
    engine.state.set_horizontal_subtile_delta(0x00);
    engine.state.set_vertical_delta(0x02);
...
fn scripted_player_sprites_draw_flip_and_blink() {
    ...
    engine.state.set_player_pose(0x30);
...
fn scripted_player_pose_uses_horizontal_delta_for_ground_flip() {
    ...
    engine.state.set_player_pose(0x30);
    engine.state.set_horizontal_subtile_delta(0xFF);
...
fn scripted_player_pose_uses_airborne_pose_while_falling() {
    ...
    engine.state.set_player_pose(0x22);
    engine.state.set_horizontal_subtile_delta(0x01);
    engine.state.set_vertical_delta(0x01);
    engine.state.set_fall_frames(0x01);
...
fn scripted_player_fall_state_counts_and_bounces_after_long_fall() {
    ...
    engine.state.set_fall_frames(0x10);
