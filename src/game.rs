...
mod queue_room_column_vram_upload {
    ...
    pub fn queue_room_column_vram_upload(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        engine
            .state
            .set_vram_addr2_hi(u8v(engine.state.vram_addr_hi() + 0x03));
...
mod advance_scripted_scroll_slice {
    ...
    pub fn advance_scripted_scroll_slice(engine: &mut Engine, r: &mut RoutineContext) {
        if (engine.state.obj_x_tile() == 0) {
            ...
            engine
                .state
                .set_vram_addr_hi(u8v((u8v((engine.state.nametable_select() ^ 0x01) << 2))
                    | engine.state.vram_addr_hi()));
            engine
                .state
                .set_obj_x_sub(u8v((u8v((((engine.state.nametable_select() ^ 0x01)
                    << 4)
                    + 0x07)))
...
mod spawn_final_exit_projectile {
    ...
    pub fn spawn_final_exit_projectile(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        engine.state.set_direction_latch(u8v(
...
mod sync_final_exit_body_slots_from_player {
    ...
    pub fn sync_final_exit_body_slots_from_player(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        engine.state.set_object_x_tile(0x20, u8v(player_tile_x + 1));
        engine.state.set_object_x_tile(0x30, u8v(player_tile_x - 2));
        engine.state.set_object_x_tile(0x10, u8v(player_tile_x - 3));
    }
}
...
mod update_scripted_player_pose_from_motion {
    ...
    fn apply_scripted_horizontal_pose(
        ...
        engine.state.set_player_pose(u8v(
...
mod draw_scripted_player_sprites {
    ...
    pub fn draw_scripted_player_sprites(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        engine
            .state
            .set_oam_attr(0x10, u8v(engine.state.player_facing() | 0x20));
        engine
            .state
            .set_oam_attr(0x14, u8v(engine.state.player_facing() | 0x20));
...
mod decode_inventory_item_list_snapshot {
    ...
    pub fn decode_inventory_item_list_snapshot(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        for progress_byte_offset in (0..=0x07).rev() {
            ...
            engine
                .state
                .set_save_progress(progress_byte_offset, u8v((high_nibble << 4) | low_nibble));
        }
        for inventory_offset in (0..=0x0F).rev() {
...
mod draw_room_object_sprites {
    ...
    pub fn draw_room_object_sprites(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        engine
            .state
            .set_oam_cursor(u8v((u8v(oam_offset + 0x38)) | 0x80));
        engine.state.set_sprite_index(u8v(object_offset + 0x10));
    }
}
...
mod upload_staged_room_view {
    ...
    pub fn upload_staged_room_view(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        engine.state.set_data_ptr_hi(u8v(
            (engine.state.data_ptr_hi() - 0x05) + engine.state.room_metadef_hi()
        ));
        upload_room_view_from_tile_pointer(engine, r);
    }
}
    ...
    pub fn resolve_room_tile_pointer(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        engine.state.set_aux_ptr_hi(engine.state.data_ptr_hi());
        {
            ...
            if ((room_offset & 0x100) != 0) {
                ...
                engine
                    .state
                    .set_aux_ptr_hi(u8v(engine.state.aux_ptr_hi() + 1));
            }
        }
        engine
            .state
            .set_data_ptr_hi(u8v(engine.state.data_ptr_hi() + 0x05));
        {
            ...
            engine.state.set_aux_ptr_hi(u8v(engine.state.aux_ptr_hi()
                + engine.state.room_metadef_hi()
                + carry));
        }
    }
}
...
mod upload_inventory_item_count_tiles {
    ...
    pub fn upload_inventory_item_count_tiles(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        if ((r.carry) == 0) {
            ...
            engine
                .state
                .set_vram_addr2_hi(u8v(engine.state.vram_addr2_hi() - 0x40));
        }
        r.value = 0x06;
        queue_ppu_job_and_wait(engine, r);
    }
...
mod handle_player_room_transition {
    ...
    pub fn handle_player_room_transition(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        engine
            .state
            .set_player_pose(u8v(engine.state.player_pose() & 0x07));
...
mod tick_player_walk_animation {
    ...
    pub fn tick_player_walk_animation(engine: &mut Engine, r: &mut RoutineContext) {
        if (engine.state.landing_timer() == 0) {
            if (engine.state.player_pose() < 0x20) {
                if ((engine.state.buttons() & 0x40) != 0) {
                    engine
                        .state
                        .set_player_pose(u8v(engine.state.player_pose() | 0x10));
                } else {
                    engine
                        .state
                        .set_player_pose(u8v(engine.state.player_pose() & 0xEF));
                }
            }
        }
        if ((engine.state.buttons() & 0x0F) == 0) {
        ...
        if ((engine.state.player_pose() & 0x08) != 0) {
            engine
                .state
                .set_player_facing(u8v(engine.state.player_facing() ^ 0x40));
        } else {
            engine
                .state
                .set_player_pose(u8v(engine.state.player_pose() ^ 0x04));
        }
    }
}
...
mod dispatch_actor_behavior {
    ...
    pub fn dispatch_actor_behavior(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        engine
            .state
            .set_indirect_ptr_lo(u8v(ACTOR_BEHAVIOR_HANDLERS[behavior_id as usize] & 0xFF));
        engine
            .state
            .set_indirect_ptr_hi(u8v(ACTOR_BEHAVIOR_HANDLERS[behavior_id as usize] >> 8));
        match behavior_id {
            0 => {
...
mod compose_large_actor_body_slots {
    ...
    pub fn compose_large_actor_body_slots(engine: &mut Engine, r: &mut RoutineContext) {
            ...
            .set_object_y_extra(0x30, engine.state.obj_y_extra());
        {
            ...
            engine.state.set_object_y_pixel(0x20, u8v(tile_y + 0x10));
            engine.state.set_object_y_pixel(0x30, u8v(tile_y + 0x10));
        }
        engine
            .state
            ...
            .set_object_x_sub(0x30, engine.state.obj_x_sub());
        {
            ...
            engine.state.set_object_x_tile(0x10, u8v(tile_x + 1));
            engine.state.set_object_x_tile(0x30, u8v(tile_x + 1));
        }
        {
            let mut actor_state: i32 = engine.state.obj_state();
...
mod dispatch_audio_stream_command {
    ...
    pub fn dispatch_audio_stream_command(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        engine
            .state
            .set_saved_audio_handler_lo(u8v(original_handler & 0xFF));
        engine
            .state
            .set_saved_audio_handler_hi(u8v(original_handler >> 8));
                ...
                0 => {
                    ...
                    loop {
                        ...
                        if (note_byte == 0) {
                            ...
                            engine
                                .state
                                .set_sound_channel_flags(u8v(
                                    engine.state.sound_channel_flags() & 0xBF
                                ));
...
mod song_init {
    ...
    pub fn song_init(engine: &mut Engine, r: &mut RoutineContext) {
        ...
        engine.state.set_song_ptr_hi(u8v(x + 1));
