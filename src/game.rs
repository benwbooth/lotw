...
pub fn tick_scripted_player_motion(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    if (directional_buttons != 0) {
        ...
        engine.state.set_direction_latch(
            (engine.state.direction_latch() & crate::bits::HIGH_NIBBLE) | engine.state.scratch0(),
        );
    }
    ...
    if (engine.state.jump_timer() == 0) && (engine.state.fall_frames() == 0) {
        ...
        engine
            .state
            .set_buttons((engine.state.buttons() & crate::bits::HIGH_NIBBLE) | crate::bits::BIT1);
    }
...
fn apply_scripted_horizontal_pose(
    ...
    engine
        .state
        .set_player_pose((engine.state.player_pose() & preserve_mask) | engine.state.scratch0());
...
pub fn stage_scrolling_intro_text_line(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    while (scan_guard < 256) {
        ...
        if (source_byte == 13) {
            ...
            let advanced_source: i32 = ((text_offset + engine.state.data_ptr_lo()) as u16 as i32);
...
pub fn draw_object_slot_sprites(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    let subtile_delta: i32 =
        ((engine.state.byte(object_base + 12)) as u16 as i32) + 256 - engine.state.scroll_fine_x();
...
pub fn select_room_data_bank_and_pointers(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    let room_table_offset: i32 = ((((((engine.state.map_screen_y() & crate::bits::BIT0) << 2) as u8
        as i32)
...
pub fn build_input_movement_delta(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    engine
        .state
        .set_player_x_velocity(((horizontal_delta & crate::bits::HIGH_NIBBLE) >> 4) | sign_fill);
...
pub fn project_player_position(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    if (horizontal_subtile_delta != 0) {
        let sum: i32 = ((horizontal_subtile_delta + engine.state.indirect_ptr_lo()) as u8 as i32);
        engine
            .state
...
pub fn collect_room_pickup_object(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    {
        let oam_offset: i32 = (((engine.state.scratch0() << 3) | crate::bits::BIT7) as u8 as i32);
        engine.state.set_oam_y(oam_offset, 239);
        engine.state.set_oam_y(4 + oam_offset, 239);
        r.index = oam_offset;
    }
...
pub fn apply_hazard_tile_contact(engine: &mut Engine, r: &mut RoutineContext) {
    let tile_ptr = ((engine.state.data_ptr()) as u16 as i32);
    let tile = engine.state.byte(((tile_ptr + r.offset) as u16 as i32)) & crate::bits::LOW_6_BITS;
    if tile != 48 {
        r.carry = 0;
        return;
    }
        ...
        match state {
            0 => {
                ...
                }
                {
                    ...
                    let mut object_slot_lo: i32 = ((engine.state.slot_index() << 4) as u8 as i32);
    ...
    'dispatch: loop {
        match state {
            0 => {
                engine
                    .state
                    .set_obj_move_state(engine.state.obj_move_state() & crate::bits::LOW_NIBBLE);
                if ((engine.state.obj_x_vel_lo() | engine.state.obj_y_vel()) != 0) {
            ...
            0 => {
                ...
                if (engine.state.obj_move_state() == 0) {
                    ...
                    if (engine.state.obj_timer() >= 8) {
                        ...
                        aim_actor_from_player_overlap(engine, r);
                        direction_diff = ((engine.state.obj_move_state() ^ engine.state.scratch0())
                            as u8 as i32);
                        changed_bits = 0;
                        bit_count = 4;
                        loop {
...
pub fn aim_actor_toward_player(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    let mut dx: i32 = ((((engine.state.obj_x_tile()) as u16 as i32) - engine.state.player_x_tile())
        as u16 as i32);
    ...
    {
        let mut dy: i32 = ((((engine.state.obj_y_pixel()) as u16 as i32) - engine.state.player_y())
            as u16 as i32);
...
pub fn project_player_projectile_position(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    if (engine.state.obj_x_vel_lo() != 0) {
        let projected_subtile: i32 = (((((engine.state.obj_x_vel_lo() << 2)
            & crate::bits::LOW_NIBBLE) as u8 as i32)
            ...
            0 => {
                ...
                loop {
                    ...
                    if ((note_byte & crate::bits::BIT7) != 0) {
                        ...
                        engine
                            .device_write(crate::engine::reg::SQ1_LO, engine.state.sound_command());
                        engine.device_write(
        ...
        match state {
            0 => {
                ...
                loop {
                    ...
                    engine.device_write(crate::engine::reg::SQ2_LO, engine.state.sound_command());
                    engine.device_write(
