/* $D1FE frame/vblank commit. The original hardware reached this work from the
 * vblank vector; the PC port calls it at explicit frame boundaries. It reads
 * PPUSTATUS into the frame status byte, triggers OAM DMA from page $02, then dispatches
 * queued VRAM jobs through the ported uploader table. */
#include "ram.h"
#include "regs.h"
...
{
    /* Preserve the ported ABI registers across the frame commit. */
    Regs save = *r;
#define COMMIT_RETURN() do { *r = save; return; } while (0)

    /* LDA PPUSTATUS -> frame status (clears the vblank latch on hardware). */
#ifdef LOTW_SHIM
    /* The frame commit begins at vblank: set the vblank flag, and sprite-0 hit
     * if the frame just rendered with rendering enabled. The $2002 read returns
     * vblank|sprite0|overflow|open-bus into the frame status byte and clears the latch. */
    {
...
#else
    RAM8(0x26) = 0x00;                    /* frame status (PPUSTATUS read; 0 in flat host) */
#endif
    REG_W(0x2003, 0x00);                  /* OAMADDR = 0 */
    REG_W(0x4014, 0x02);                  /* OAMDMA from page $02 */

    u8 req = RAM8(0x28);                  /* LDA vblank_vram_req */
    if (req == 0) {                       /* BEQ L_D21E */
        vblank_commit_tail(r);
        COMMIT_RETURN();
    }
    RAM8(0x28) = 0x00;                    /* LDX #$00 / STX vblank_vram_req — the real code
                                           ...
                                           * wait on $28.) */
    if (req >= 0x07) {                    /* CMP #$07 / BCC L_D221 — else fall to tail */
        vblank_commit_tail(r);
        COMMIT_RETURN();
    }
    ...
    /* each vram_* tail-calls vblank_commit_tail */
    *r = save;
#undef COMMIT_RETURN
}
