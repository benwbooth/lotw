/* $D2E5 vram_blit_stack — vblank VRAM job: blit 4*16 = 64 bytes pulled off the
 * stack page (SP repointed to $FF) straight to PPUDATA, restore SP, return
 * through vblank_commit_tail. INSPECTION-PORT: manipulates the stack pointer (not modelled
...
void vram_blit_stack(Regs *r)
{
     ...
     * model the old call stack, but the game-visible stack page bytes are in
