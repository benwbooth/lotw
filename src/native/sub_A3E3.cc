/* $A3E3 — warp / between-levels teleport cutscene driver, far-called from
 * main_init / main_loop. It is the largest routine in the game.
 *
 ...
 *     companion OAM bookkeeping pass runs when bit6 of the frame status byte is set
 ...
 *     on the vblank PPU-job queue $CC8F = queue_ppu_job_and_wait), animates the web
 ...
 *
 * ABI: void sub_A3E3(Regs *r). Legacy accumulator/index values and flags travel
 * in r; memory is RAM8()/REG_W() over the flat 64K space. Wait loops on the
 * frame-decremented $36 counter and on the frame-cleared queue flag are kept as
 ...
 * ($D42B), $C15D.
 *
 * INSPECTION-PORT (no diff-test spec): a far-call-driven, frame-synchronised
 * multi-second cutscene (PPU-job queue waits, frame counter $36 waits, OAM/
 * scroll register effects via callees) that cannot be exercised in flat host
 * memory — the frame commit never runs, so $36 never decrements and the PPU-job flag
...
static void farcall_cce4(Regs *r, u8 lo, u8 hi, PortFn target)
{
...
static lotw::native::FrameTask cutscene_a7ff(Regs *r)
{
    ...
    for (;;) {                                  /* L_A81E: walk player up */
        ...
        sub_AD7A(r);
        game.set_frame_counter(0x01);           /* L_A82D: vblank wait */
    ...
    do {                                        /* L_A880: scroll loop ($10 frames) */
        ...
        if (game.frame_status_bit6_set()) {     /* BIT frame status / BVC L_A8A5 */
    ...
    RAM8(0x1D) = 0x01;
    r->a = 0xFF; queue_ppu_job_and_wait(r);     /* LDA #$FF / JSR $CC8F */
    if (game.player_health() == 0) co_return;   /* LDA health / BNE L_A8C5 / RTS (death) */
    ...
    do { sub_AAEE(r); } while (game.countdown_timer_active());/* L_A9CD */
    ...
    for (;;) {                                  /* L_A9D4 — raise player to $A0 */
        ...
        sub_AAEE(r);
        sub_AAEE(r);
        lotw::native::commit_frame_work(r);     /* JSR $C135 (frame commit) */
        while (game.frame_counter_active())
            co_yield lotw::native::Wait::next_frame();
        /* JMP L_A9D4 */
    }
    ...
    do { sub_AAEE(r); } while (game.countdown_timer_active());/* L_AA0A */
    ...
    for (;;) {                                  /* L_AA11 — scroll columns until player_x_tile==$37 */
        ...
        /* L_AA44 */
        lotw::native::commit_frame_work(r);     /* JSR $C135 commit */
        while (game.frame_counter_active())
            co_yield lotw::native::Wait::next_frame();
    ...
    do {                                        /* L_AA69 */
        ...
        sub_AAEE(r); sub_AAEE(r); sub_AAEE(r); sub_AAEE(r);
    } while (game.countdown_timer_active());     /* BNE L_AA69 */
...
extern "C" void sub_A3E3(Regs *r)
{
    ...
    if (RAM8(0x00F2) == 0) {                    /* LDA boss_life / BNE L_A3F5 */
        lotw::native::run_frame_task(
            r,
            cutscene_a7ff(r),
            [] { return std::uint8_t{0}; });    /* JMP L_A7FF */
        return;
    }

    /* L_A3F5 — OAM particle / boss-damage bookkeeping when frame status bit6 set. */
    if (game.frame_status_bit6_set()) {         /* BIT frame status / BVC L_A43C */
...
phase4:                                          /* L_A549 */
    ...
    if (RAM8(0xE9) == 0x04) game.set_prompt_state(0x20); /* CMP #$04 / BNE L_A557 */
    /* L_A557 */
    RAM8(0x7A) = 0xB5;
    RAM8(0x1E) = 0xC2;
    goto draw;                                    /* JMP L_A56D */
