 ...
 * $F628,$E87C,$F782,$C15D.
 *
 * INSPECTION-PORT (no diff-test spec): PPU/MMC3 register setup, frame-counter
 * waits, controller input loops, and a far-call-driven title/select sequence.
 * These need whole-port integration because frame commits drive the timers and
 * input state. Integration-verified. */
...
void sub_C569(Regs *r); void sub_C38B(Regs *r);
...
static lotw::native::FrameTask sub_AE64_script(Regs *r)
{
...
restart:                                            /* L_AE64 */
    ...
    game.set_frame_counter(0x78);                    /* L_AEBE: ~120-frame title hold */
    ...
    for (;;) {
        ...
        pad = lotw::native::read_buttons(r);        /* JSR $CC43 */
        if (pad == 0xFF) {                          /* CMP #$FF / BNE */
            game.set_prompt_state(0x1A);
            RAM8(0x37) = 0x1A;
        }
        (void)pad;
        if (game.buttons() & 0x10) {                /* AND #$10 / BNE -> JMP L_B0B1 */
            sub_B0B1(r);
            co_return;
        }
        if (game.button_chord() == 0x83) {          /* CMP #$83 / BEQ -> JMP L_B13D */
            sub_B13D(r);
            co_return;
        }
        ...
        RAM8(0x0E) = 0x35; RAM8(0x0F) = 0xC1;       /* far-call $C135 (commit frame) */
        RAM8(0x30) = RAM8(0x32); RAM8(0x31) = RAM8(0x33); RAM8(0x25) = 0x06; NES_PRG_SYNC();
        lotw::native::commit_frame_work(r);
        while (game.frame_counter_active())
            co_yield lotw::native::Wait::next_frame();
        RAM8(0x30) = 0x0C; RAM8(0x31) = 0x0D; RAM8(0x25) = 0x07; NES_PRG_SYNC();
        if (game.countdown_timer_active()) continue;/* BNE L_AEC9 */
        break;                                      /* JMP L_AF1D */
    }
    ...
    for (;;) {
        ...
        sub_B11A(r);
        lotw::native::read_buttons(r);              /* JSR $CC43 */
        if (game.buttons() & 0x10) {                /* AND #$10 / BEQ else JMP L_B0B1 */
            sub_B0B1(r);
            co_return;
        }
        /* L_B036 */
        game.set_buttons(RAM8(0xFE));               /* LDA $FE / STA $20 */
        {
            int do_b044 = 1;
        ...
        RAM8(0x0E) = 0x35; RAM8(0x0F) = 0xC1;       /* far-call $C135 */
        RAM8(0x30) = RAM8(0x32); RAM8(0x31) = RAM8(0x33); RAM8(0x25) = 0x06; NES_PRG_SYNC();
        lotw::native::commit_frame_work(r);
        while (game.frame_counter_active())
            co_yield lotw::native::Wait::next_frame();
        RAM8(0x30) = 0x0C; RAM8(0x31) = 0x0D; RAM8(0x25) = 0x07; NES_PRG_SYNC();
        if (game.countdown_timer_active()) continue;/* BNE L_B021 */
        break;                                       /* L_B0A6 */
    }
    /* L_B0A6 */
    sub_C461(r);
