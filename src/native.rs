...
fn item_spawn_setup(engine: &mut Engine, r: &mut RoutineContext, x: i32) {
    engine.state.set_obj_state(x + 2);
    engine.state.set_obj_tile((x << 2) | crate::bits::BITS_0_7);
...
pub fn queue_ppu_job_and_wait(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    engine.state.set_nmi_vram_req(((r.value) as i32));
    frame::wait_for_ppu_job_idle(engine, r);
}
...
pub fn main_loop_dispatch(engine: &mut Engine, r: &mut RoutineContext) {
    loop {
        ...
        if engine.state.player_health == 0 {
            ...
            r.index = (((r.index - 1) as u8 as i32) as u8);
        ...
        if engine.state.final_exit_flag() != 0 {
            ...
            engine
                .state
                .set_player_x_fine(engine.state.player_x_fine() & crate::bits::LOW_NIBBLE);
        ...
        if ((r.carry) == 0) && engine.state.saved_scroll_tile() != engine.state.scroll_tile_x() {
            engine
                .state
                .set_main_loop_phase((engine.state.main_loop_phase() + 1) & crate::bits::BYTE_MASK);
        }
...
pub fn setup_final_exit_sequence(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    engine.state.set_prompt_state(32);
    engine.state.set_frame_counter(60);
    frame::commit_frame_work(engine, r);
    frame::wait_for_frame_counter(engine, r);

    engine.state.set_map_screen_y(19);
    engine.state.set_map_screen_x(2);
    farcall_cce4(engine, r, 242, 200, crate::game::scene_assemble);
    crate::game::clear_name_tables_to_blank_tiles(engine, r);

    engine.state.set_oam_y(0, 239);
    engine.state.set_scroll_y(34);
    ...
    loop {
        ...
        engine
            .state
            .set_nametable_select((x & crate::bits::BIT3) >> 3);
        r.value = 255;
        queue_ppu_job_and_wait(engine, r);
    }
    ...
    engine.state.set_sprite_index(8);
    engine
        .state
        .set_player_x_fine((engine.state.player_x_tile() << 4) | engine.state.player_x_fine());
...
fn run_final_exit_cutscene(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    engine.state.set_obj_health(0);
    engine.state.set_sprite_blink_timer(0);
    engine.state.set_displaced_timer(0);
    crate::game::draw_scripted_player_sprites(engine, r);
    ...
    while engine.state.player_y() < 160 {
        engine
            .state
            .set_player_y((engine.state.player_y() + 1) & crate::bits::BYTE_MASK);
        crate::game::draw_scripted_player_sprites(engine, r);
        engine.state.set_frame_counter(1);
        frame::wait_for_frame_counter(engine, r);
    }
    ...
    engine.state.set_prompt_state(32);
    engine.state.set_prompt_argument(128);
    engine.state.set_tile_table_ptr_hi(182);
    ...
    engine.state.set_prompt_state(32);
    engine.state.set_prompt_argument(128);
    engine.state.set_tile_table_ptr_hi(183);
    loop {
        crate::game::advance_scripted_scroll_slice(engine, r);
        if engine.state.obj_x_tile() == 0 {
            break;
    ...
    engine.state.set_tile_fetch_counter(0);
    loop {
        ...
        if engine.state.sprite_index() == 0 {
            engine.state.set_sprite_index(2);
        }

        crate::game::draw_scripted_player_sprites(engine, r);
        crate::game::rotate_sprite_zero_from_scripted_oam(engine, r);
        engine.state.set_tile_fetch_counter(
            (engine.state.tile_fetch_counter() - 1) & crate::bits::BYTE_MASK,
        );
    ...
    engine.state.set_oam_y(16, 239);
    engine.state.set_oam_y(20, 239);
    engine.state.set_sprite_index(0);
    engine.state.set_oam_cursor(128);
    crate::game::reset_room_object_slots(engine, r);
    ...
    engine.state.set_player_y(192);
    engine.state.set_player_x_tile(26);
    engine.state.set_player_x_fine(1);
    engine.state.set_scroll_fine_x(1);
    engine.state.set_player_pose(9);
    ...
    crate::game::draw_room_object_sprites(engine, r);
    engine.state.set_character_index(7);
    farcall_cce4(engine, r, 146, 196, fade_room_palette_in);
    engine.state.set_countdown_timer(5);
    ...
    crate::game::draw_player_sprites(engine, r);
    engine.state.set_countdown_timer(3);
    while engine.state.countdown_timer_active() {
        draw_scene_and_wait_one_frame(engine, r);
    }
    ...
    engine.state.set_player_pose(25);
    ...
    engine.state.set_countdown_timer(20);
    while engine.state.countdown_timer_active() {
        engine
            .state
...
pub fn tick_final_exit_sequence(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    if engine.state.sprite0_hit() {
        let t = (engine.state.sprite_index() + 2) & crate::bits::BITS_1_2;
        if t != 0 {
            let x = ((t << 3) as u8 as i32);
            if engine.state.object_state(x) != 0 {
                engine.state.set_object_state(x, 0);
                let sum =
                    ((engine.state.scroll_pixel_x() + engine.state.object_x_sub(x)) as u8 as i32);
                if sum >= 176 && sum < 208 {
                    let bl = engine.state.obj_health();
                    engine
                        .state
                        .set_obj_health(if bl < 2 { 0 } else { ((bl - 2) as u8 as i32) });
                    crate::game::build_object_health_meter_standard_tiles(engine, r);
                    engine.state.set_prompt_state(32);
        ...
        match engine.state.obj_timer() {
            4 => {
                engine.state.set_scheduler_phase(
                    (engine.state.scheduler_phase() - 1) & crate::bits::BYTE_MASK,
                );
                if engine.state.scheduler_phase() != 0 {
                    if engine.state.scheduler_phase() == 4 {
                        engine.state.set_prompt_state(32);
                    }
                    engine.state.set_tile_table_ptr_hi(181);
                    engine.state.set_scroll_y(194);
                } else {
                    engine.state.set_tile_table_ptr_hi(179);
            ...
            3 => {
                ...
                if engine.state.scheduler_phase() != 0 {
                    engine.state.set_tile_table_ptr_hi(178);
                    if engine.state.scroll_pixel_x() != 0 {
                        ...
                        if v >= 17 {
                            if engine.state.scroll_y() < 210 {
                                engine.state.set_scroll_y(
                                    (engine.state.scroll_y() + 4) & crate::bits::BYTE_MASK,
                                );
                            } else if engine.state.scroll_pixel_x() != 0 {
                                engine.state.set_scroll_pixel_x(
                                    (engine.state.scroll_pixel_x() - 4) & crate::bits::BYTE_MASK,
                                );
                            }
                        ...
                        engine
                            .state
                            .set_scroll_y((engine.state.scroll_y() - 4) & crate::bits::BYTE_MASK);
                    }
                } else if engine.state.scroll_pixel_x() != 0 {
                    engine.state.set_obj_timer(0);
                } else {
                    engine.state.set_tile_table_ptr_hi(176);
                    engine
                        .state
                        .set_obj_timer((engine.state.obj_timer() + 1) & crate::bits::BYTE_MASK);
                    engine.state.set_scheduler_phase(4);
                }
            }
            2 => {
                engine.state.set_scheduler_phase(
                ...
                if engine.state.scheduler_phase() != 0 {
                    engine.state.set_tile_table_ptr_hi(180);
            ...
            1 => {
                ...
                if engine.state.scheduler_phase() == 0 {
                    ...
                    let a = ((((engine.state.scheduler_phase() << 1) & crate::bits::BIT0) + 176)
                        as u8 as i32);
            ...
            _ => {
                let sum =
                    ((engine.state.scroll_pixel_x() + engine.state.player_x_fine()) as u8 as i32);
                ...
                if close || (delayed_grow && engine.state.scroll_y() >= 195) {
                    engine.state.set_obj_timer(3);
                    engine.state.set_scheduler_phase(2);
                    engine.state.set_scheduler_phase(
                        (engine.state.scheduler_phase() - 1) & crate::bits::BYTE_MASK,
                    );
                    if engine.state.scheduler_phase() != 0 {
                    ...
                    engine.state.set_obj_timer(2);
                    engine.state.set_scheduler_phase(8);
                    engine.state.set_tile_table_ptr_hi(179);
                } else {
                    engine.state.set_obj_timer(1);
                    engine.state.set_scheduler_phase(4);
                    engine.state.set_scheduler_phase(
                        (engine.state.scheduler_phase() - 1) & crate::bits::BYTE_MASK,
                    );
...
pub fn fade_partial_palette_buffer_out(engine: &mut Engine, r: &mut RoutineContext) {
    let mut y = 4;
    loop {
        engine.state.set_frame_counter(5);
        for x in (0..=12).rev() {
...
pub fn run_story_text_sequence(engine: &mut Engine, r: &mut RoutineContext) {
    engine.state.set_music_volume_override(
        (engine.state.music_volume_override() + 1) & crate::bits::BYTE_MASK,
    );
    drain_audio_timers_with_object_frames(engine, r);
    fade_palette_buffer_out(engine, r);
    crate::game::clear_name_tables_to_blank_tiles(engine, r);
    ...
    engine
        .state
        .set_ppu_mask_shadow(engine.state.ppu_mask_shadow() | crate::bits::BITS_3_4);

    r.value = 255;
    queue_ppu_job_and_wait(engine, r);

    engine.state.set_song(10);
    crate::game::song_init(engine, r);

    engine.state.set_scroll_pixel_x(0);
    engine.state.set_nametable_select(0);
    engine.state.set_scratch2(0);
    engine.state.set_scroll_fine_x(0);
    engine.state.set_scroll_tile_x(0);
    crate::game::load_intro_text_palette(engine, r);

    engine.state.set_vram_addr2_lo(64);
    engine.state.set_vram_addr2_hi(1);
    engine.state.set_inventory_upload_col(32);
    engine.state.set_data_ptr_lo(156);
    engine.state.set_data_ptr_hi(183);
...
pub fn run_title_screen_loop(engine: &mut Engine, r: &mut RoutineContext) {
    'restart: loop {
        ...
        engine.state.set_ppu_ctrl_shadow(160);
        engine.device_write(crate::engine::reg::PPU_CTRL, 160);
        engine.state.set_ppu_mask_shadow(0);
        engine.device_write(crate::engine::reg::PPU_MASK, 0);
        engine.state.set_scroll_pixel_x(0);
        engine.state.set_nametable_select(0);
        engine.state.set_scroll_y(232);
        for x in (0..=31).rev() {
        ...
        engine.state.set_ppu_mask_shadow(30);
        engine.device_write(crate::engine::reg::PPU_MASK, 30);
        engine.state.set_frame_counter(120);
        frame::wait_for_frame_counter(engine, r);
        ...
        loop {
            ...
            if pad == 255 {
                ...
                engine.state.set_continue_timer(26);
            }
            ...
            if (engine.state.frame_prescaler() & crate::bits::LOW_3_BITS) == 0 {
                ...
                engine
                    .state
                    .set_palette_buffer(2, hi | engine.state.scratch0());
            }
            enter_return_home(engine, 53, 193);
            frame::commit_frame_work(engine, r);
            frame::wait_for_frame_counter(engine, r);
        ...
        engine.state.set_map_screen_x(((r.value) as i32));
        r.value = 16;
        crate::game::rng_update(engine, r);
        engine.state.set_map_screen_y(((r.value) as i32));
        farcall_cce4(engine, r, 242, 200, crate::game::scene_assemble);

        loop {
            r.value = 64;
            crate::game::rng_update(engine, r);
            engine.state.set_player_x_tile(((r.value) as i32));
            engine.state.set_data_ptr_lo(((r.value) as i32));
            engine.state.set_player_x_fine(0);
            r.value = 11;
            crate::game::rng_update(engine, r);
            r.value = (((r.value << 4) as u8 as i32) as u8);
            engine.state.set_player_y(((r.value) as i32));
            engine.state.set_data_ptr_hi(((r.value) as i32));
        ...
        engine.state.set_character_index(((chr) as i32));
        ...
        engine
            .state
            .set_chr_bank(2, engine.state.character_index() + 56);
        ...
        crate::game::draw_status_item_sprites(engine, r);
        farcall_cce4(engine, r, 146, 196, fade_room_palette_in);
        engine.state.set_countdown_timer(10);
        ...
        loop {
            ...
            if (engine.state.horizontal_subtile_delta() | engine.state.vertical_delta()) != 0 {
                engine
                    .state
                    .set_title_timer((engine.state.title_timer() - 1) & crate::bits::BYTE_MASK);
                if engine.state.title_timer() != 0 {
                    do_b044 = false;
                }
            }
            if do_b044 {
                engine.state.set_title_timer(128);
            ...
            crate::game::draw_room_object_sprites(engine, r);
            if engine.state.saved_scroll_tile() != engine.state.scroll_tile_x() {
                engine.state.set_main_loop_phase(
                    (engine.state.main_loop_phase() + 1) & crate::bits::BYTE_MASK,
                );
            }
            enter_return_home(engine, 53, 193);
            frame::commit_frame_work(engine, r);
...
pub fn drain_audio_timers_with_object_frames(engine: &mut Engine, r: &mut RoutineContext) {
    engine.state.set_sound_channel_byte(1, 32, 0);
    engine.state.set_data_ptr_hi(16);
    loop {
        ...
        engine.state.set_data_ptr_lo(20);
        loop {
            crate::game::draw_room_object_sprites(engine, r);
            engine.state.set_frame_counter(1);
            frame::commit_frame_work(engine, r);
            ...
            engine
                .state
                .set_data_ptr_lo((engine.state.data_ptr_lo() - 1) & crate::bits::BYTE_MASK);
        ...
        engine
            .state
            .set_data_ptr_hi((engine.state.data_ptr_hi() - 1) & crate::bits::BYTE_MASK);
...
pub fn run_player_death_or_continue_flow(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    engine.state.set_frame_counter(1);
    engine.state.set_player_pose(49);
    crate::game::draw_player_sprites(engine, r);
    frame::commit_frame_work(engine, r);
    frame::wait_for_frame_counter(engine, r);
    ...
    if !use_game_over_screen {
        if (engine.state.continue_timer() & crate::bits::BIT7) != 0 {
            let x = engine.state.selected_item_slot();
            if engine.state.item_slot(x) == 12 {
                engine.state.set_item_slot(x, 255);
                crate::game::draw_status_item_sprites(engine, r);
            } else {
            ...
            engine
                .state
                .set_continue_timer((engine.state.continue_timer() + 1) & crate::bits::BYTE_MASK);
        }
    ...
    fade_palette_buffer_out(engine, r);
    engine.state.set_final_exit_flag(0);
    engine.state.set_sprite_index(0);
    engine.state.set_oam_cursor(128);
    crate::game::clear_name_tables_to_blank_tiles(engine, r);
    crate::game::reset_room_object_slots(engine, r);
    ...
    engine.state.set_player_x_tile(5);
    engine.state.set_player_x_fine(0);
    engine.state.set_player_y(112);
    engine.state.set_player_pose(57);
    crate::game::clear_oam_with_sprite_zero_template(engine, r);
    crate::game::draw_player_sprites(engine, r);
    ...
    loop {
        ...
        engine
            .state
            .set_player_y(engine.state.player_y() ^ crate::bits::BIT4);
    ...
    engine.state.set_character_index(6);
    engine.state.set_map_screen_x(3);
    engine.state.set_map_screen_y(16);
    fade_palette_buffer_out(engine, r);
    engine.state.set_song(2);
    crate::game::clear_name_tables_to_blank_tiles(engine, r);
    crate::game::upload_status_panel_template(engine, r);
    crate::game::sync_health_hud(engine, r);
    crate::game::sync_magic_hud(engine, r);
...
pub fn show_player_pose_for_eight_frames(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    engine.state.set_frame_counter(8);
    crate::game::draw_player_sprites(engine, r);
    frame::commit_frame_work(engine, r);
    frame::wait_for_frame_counter(engine, r);
}
...
pub fn fade_title_palette_in(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    loop {
        ...
        engine
            .state
            .set_scratch1((engine.state.scratch1() - 16) & crate::bits::BYTE_MASK);
        if (engine.state.scratch1() & crate::bits::BIT7) != 0 {
            break;
        }
    }
    crate::game::upload_palette_buffer(engine, r);
...
pub fn fade_room_palette_row_in(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    loop {
        ...
        frame::commit_frame_work(engine, r);
        frame::wait_for_frame_counter(engine, r);
        v = ((engine.state.scratch1() - 16) as u8 as i32);
        engine.state.set_scratch1(v);
        if (v & crate::bits::BIT7) != 0 {
            break;
        }
    }
    crate::game::upload_palette_buffer(engine, r);
...
pub fn find_damageable_actor_overlap(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    let mut x = 144;
    loop {
        let mut skip = ((y) as u8 as i32) == engine.state.slot_index();
        ...
        if !skip {
            let mut d = ((engine.state.scratch2() - engine.state.object_y_pixel(x)) as u8 as i32);
            if !(d < 16) && d < 241 {
                skip = true;
            }
            if !skip {
                d = ((engine.state.indirect_ptr_hi() - engine.state.object_x_tile(x)) as u8 as i32);
                if d == 0 {
                    engine.state.set_scratch0(y);
...
pub fn update_player_terrain_contact(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    engine.state.set_scratch2(engine.state.player_y() + 1);
    crate::game::resolve_room_tile_pointer(engine, r);

    if engine.state.player_x_fine() == 0 {
        engine.state.set_pose_state(1);
        r.offset = 0;
    ...
    if engine.state.player_y() >= 176 {
        engine
            .state
            .set_fall_frames((engine.state.fall_frames() + 1) & crate::bits::BYTE_MASK);
        return;
    }
...
fn resolve_player_landing_or_hazard_contact(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    if fall_frames >= engine.state.jump_strength() {
        ...
        engine.state.set_landing_timer(fall_frames + 10);
...
pub fn dispatch_room_tile_action(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    if tile == engine.state.text_attr_ptr_lo() {
        if engine.state.object_state(144) == 0 {
            engine.state.set_scratch3(((tile_offset) as i32));
            engine.state.set_obj_tile(225);
            engine.state.set_obj_state(1);
            engine.state.set_obj_attr(1);
            engine
                .state
                .set_obj_move_scratch(engine.state.text_attr_ptr_hi());
            engine.state.set_obj_timer(10);
            crate::game::seed_object_position_from_tile_offset(engine, r);
            crate::game::store_object_slot_scratch(engine, r);
            engine.state.set_prompt_state(6);
        }
        let v = engine.state.text_attr_ptr_hi() & crate::bits::LOW_6_BITS;
        r.value = ((v) as u8);
        r.carry = (((v >= 48) as u8 as i32) as u8);
        return;
    }
    if tile == 2 {
        if engine.state.object_state(144) == 0 {
    ...
    if tile == 62 {
        if (engine.state.buttons() & crate::bits::BIT7) != 0 && engine.state.object_state(144) == 0
        {
            engine.state.set_scratch3(((tile_offset) as i32));
            ...
            if idx == 1 {
                if engine.state.player_magic != 0 {
                    ...
                    if t == 0 {
                        let x2 = (((engine.state.direction_latch() & crate::bits::LOW_NIBBLE) << 1)
                            as u8 as i32);
                        ...
                        if b == 62 {
                            ...
                            engine
                                .state
                                .set_object_move_scratch(144, ((r.value) as i32));
                            crate::game::consume_magic_point(engine, r);
                            engine.state.set_prompt_state(20);
                        }
                    }
                }
                r.carry = 1;
            ...
            if idx == 2 {
                if (engine.state.direction_latch() & crate::bits::LOW_NIBBLE) != 0 {
                    ...
                    engine.state.set_obj_tile(
                        engine.state.byte(((p79 + 248) as u16 as i32)) & crate::bits::CLEAR_BIT0,
                    );
                    engine.state.set_obj_state(1);
                    engine.state.set_obj_attr(3);
                    r.offset = ((engine.state.scratch3()) as u8);
                    let b = engine
                        .state
                        .byte(((tile_ptr + ((r.offset) as i32)) as u16 as i32));
                    engine.state.set_obj_move_scratch(b);
                    engine.state.set_obj_timer(16);
                    crate::game::read_room_tile_action_value(engine, r);
            ...
            if idx == 3 {
                if engine.state.player_magic != 0 {
                    if (engine.state.direction_latch() & crate::bits::LOW_NIBBLE) != 0 {
                        ...
                        let p79 = ((engine.state.tile_table_ptr()) as u16 as i32);
                        engine.state.set_obj_tile(
                            engine.state.byte(((p79 + 248) as u16 as i32))
...
pub fn animate_health_refill_to_cap(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    engine.state.set_prompt_state(23);
...
pub fn unlock_door_with_key(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    engine.state.set_prompt_state(31);
    crate::game::draw_room_object_sprites(engine, r);
...
pub fn run_character_select_overlay(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    if engine.state.chr_bank(3) < 48 {
        ...
        engine.state.set_scroll_fine_x(8);
        crate::game::refresh_scroll_register_shadows(engine, r);
        crate::game::draw_player_sprites(engine, r);
...
pub fn show_inventory_item_list_screen(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    engine.state.set_indirect_ptr_lo(212);
    engine.state.set_indirect_ptr_hi(180);
...
pub fn run_inventory_item_grid_menu(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    loop {
        ...
        if (engine.state.buttons() & crate::bits::CLEAR_BITS_4_5) != 0 {
            engine.state.set_prompt_state(12);
            engine.state.set_frame_counter(10);
        }
        frame::commit_frame_work(engine, r);
        frame::wait_for_frame_counter(engine, r);
    }
...
pub fn run_character_select_room_flow(engine: &mut Engine, r: &mut RoutineContext) {
    if engine.state.map_screen_y() != 16 {
        ...
        loop {
            ...
            loop {
                engine
                    .state
                    .set_coins((engine.state.coins() - 1) & crate::bits::BYTE_MASK);
                crate::game::sync_coin_hud(engine, r);
                engine.state.set_prompt_state(12);
                engine.state.set_frame_counter(10);
                frame::commit_frame_work(engine, r);
    ...
    engine.state.set_selected_item_slot(3);
    crate::game::draw_status_item_sprites(engine, r);
    engine.state.set_player_pose(241);
    engine.state.set_player_facing(0);
    crate::game::draw_player_sprites(engine, r);
    crate::game::restore_status_sprite_template(engine, r);
    crate::game::reset_room_object_slots(engine, r);
    ...
    loop {
        ...
        let hi = engine.state.scratch2() & crate::bits::HIGH_NIBBLE;
        let mut chosen: Option<i32> = None;
        if hi == 80 {
            if (engine.state.indirect_ptr_hi() & crate::bits::LOW_NIBBLE) == 5
                && engine.state.continue_timer() != 0
            {
                    ...
                    && engine.state.buttons() == 195
                {
                    ...
                    engine.state.set_continue_timer(128);
                    engine.state.set_coins(128);
                    engine.state.set_keys(128);
                    engine.state.set_prompt_state(26);
                }
            }
            continue;
        } else if hi == 112 {
            let lo = engine.state.indirect_ptr_hi() & crate::bits::LOW_NIBBLE;
            if lo == 6 {
                chosen = Some(0);
            } else if lo == 8 {
                chosen = Some(1);
        ...
        r.offset = ((((x << 2) + 3) as u8 as i32) as u8);
        for xi in (0..=3).rev() {
            engine.state.set_item_slot(
                11 + xi,
                engine
            ...
            r.offset = (((r.offset - 1) as u8 as i32) as u8);
        }
        engine.state.set_prompt_state(24);
        engine.state.set_prompt_argument(255);
        engine.state.set_frame_counter(4);
        frame::commit_frame_work(engine, r);
        frame::wait_for_frame_counter(engine, r);
        ...
        engine
            .state
            .set_player_y(engine.state.player_y() & crate::bits::HIGH_NIBBLE);
        engine.state.set_player_x_fine(4);
        crate::game::clear_gameplay_object_sprites(engine, r);
        crate::game::draw_player_sprites(engine, r);
        frame::commit_frame_work(engine, r);
        frame::wait_for_frame_counter(engine, r);
...
pub fn run_shop_room_flow(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    loop {
        ...
        let nib = engine.state.player_x_tile() & crate::bits::LOW_NIBBLE;
        let x = if nib < 3 {
            continue;
        } else if nib < 5 {
            0
        ...
        if (item & crate::bits::BIT7) != 0 {
            ...
            if ((r.carry) != 0) {
                ...
                if item == 13 && engine.state.continue_timer() != 0 {
                    engine.state.set_shop_active(1);
                }
                engine.state.set_prompt_state(6);
            }
        }
...
pub fn tick_special_exit_actor_sequence(engine: &mut Engine, r: &mut RoutineContext) {
    if (engine.state.obj_state() & crate::bits::LOW_7_BITS) == 0 {
        engine.state.set_prompt_state(24);
        engine.state.set_prompt_argument(255);
        r.index = 3;
        flash_palette_buffer(engine, r);
        ...
        engine
            .state
            .set_obj_state((engine.state.obj_state() + 1) & crate::bits::BYTE_MASK);
        engine.state.set_prompt_state(2);
        engine.state.set_obj_cooldown(15);
        engine.state.set_obj_x_vel_lo(0);
    ...
    if engine.state.obj_move_scratch() == 0 {
        engine
            .state
            .set_obj_cooldown((engine.state.obj_cooldown() - 1) & crate::bits::BYTE_MASK);
        if engine.state.obj_cooldown() == 0 {
            engine
                .state
                .set_obj_attr(engine.state.obj_attr() | crate::bits::BIT7);
            engine.state.set_obj_move_scratch(1);
            return;
        }
        let a = ((((engine.state.obj_cooldown() >> 2) ^ crate::bits::BYTE_MASK) + 1) as u8 as i32);
    ...
    engine
        .state
        .set_obj_move_scratch((engine.state.obj_move_scratch() + 1) & crate::bits::BYTE_MASK);
    engine
        .state
        .set_obj_y_vel((engine.state.obj_move_scratch() >> 2) + 1);
    crate::game::project_actor_position(engine, r);
    crate::game::check_position_out_of_bounds(engine, r);
    if ((r.carry) != 0) {
        engine.state.set_obj_state(0);
        engine.state.set_obj_timer(240);
        engine.state.set_pending_special_exit(1);
        return;
    }
    engine.state.set_obj_y_pixel(engine.state.scratch2());
}
...
pub fn run_carried_item_loadout_flow(engine: &mut Engine, r: &mut RoutineContext) {
    loop {
        ...
        if ((r.carry) != 0) {
            ...
            if engine.state.item_slot(e) == 13 {
                engine.state.set_selected_item_slot(3);
                crate::game::draw_status_item_sprites(engine, r);
            }
            return;
        ...
        let flow_0441 = if py >= 88 {
            ...
            x = (((engine.state.player_x_tile() >> 1) | engine.state.scratch0()) as u8 as i32);
...
pub fn tick_defeated_actor_reward_drop(engine: &mut Engine, r: &mut RoutineContext) {
    ...
    if (engine.state.obj_state() & crate::bits::LOW_7_BITS) == 0 {
        engine
            .state
            .set_obj_state((engine.state.obj_state() + 1) & crate::bits::BYTE_MASK);
        engine.state.set_prompt_state(14);
        engine.state.set_obj_cooldown(8);
        engine.state.set_obj_x_vel_lo(0);
        engine.state.set_obj_x_vel_hi(0);
        engine.state.set_obj_move_scratch(0);
        engine.state.set_obj_y_extra(engine.state.obj_y_pixel());
        let ptr = ((engine.state.actor_record_ptr_lo() | (engine.state.actor_record_ptr_hi() << 8))
            as u16 as i32);
        engine
            .state
            .set_obj_tile(engine.state.byte(((ptr + 6) as u16 as i32)));
        engine
            .state
            .set_obj_attr(engine.state.obj_attr() & crate::bits::LOW_2_BITS);
    }
    if engine.state.obj_move_scratch() == 0 {
        engine
            .state
            .set_obj_cooldown((engine.state.obj_cooldown() - 1) & crate::bits::BYTE_MASK);
        if engine.state.obj_cooldown() != 0 {
            engine.state.set_obj_y_vel(0 - engine.state.obj_cooldown());
            crate::game::project_actor_position(engine, r);
            crate::game::check_position_out_of_bounds(engine, r);
            if ((r.carry) == 0) {
                engine.state.set_obj_y_pixel(engine.state.scratch2());
