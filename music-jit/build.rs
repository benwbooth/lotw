//! Pick the song source compiled into the JIT cdylib. By default it's the one real
//! `src/music/songs.rs`; the music-server sets `LOTW_JIT_SONGS` to the live editor
//! buffer so live-editing compiles that instead. The chosen source is copied to
...
fn main() {
    ...
    let src = std::env::var("LOTW_JIT_SONGS").unwrap_or_else(|_| format!("{manifest}/../src/music/songs.rs"));
